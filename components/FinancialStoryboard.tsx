...
export default function FinancialStoryboard() {
  const [events, setEvents] = useState<FinancialEvent[]>([
    ...
    },
    {
      ...
      description: 'Purchased additional investment property',
      type: 'positive',
    ...
    },
    {
      ...
      description: 'Digital assets experienced market correction',
      type: 'negative',
  ...
  const projectedData = useMemo(() => {
    ...
    let lastValue =
      OVER_TIME[OVER_TIME.length - 1].Cash +
      OVER_TIME[OVER_TIME.length - 1].Equities +
      OVER_TIME[OVER_TIME.length - 1].Trust +
      OVER_TIME[OVER_TIME.length - 1].Digital +
      OVER_TIME[OVER_TIME.length - 1].Property;

    const projectionMonthGrowth = 25000;

    return months.map((month) => {
      lastValue += projectionMonthGrowth;
      return {
        month,
        projected: lastValue,
        isProjection: true,
  ...
  const combinedData = useMemo(() => {
    const historical = OVER_TIME.map((item) => ({
      month: item.month,
      actual:
        item.Cash + item.Equities + item.Trust + item.Digital + item.Property,
      isProjection: false,
    }));
  ...
  const handleAddEvent = () => {
    if (newEvent.month && newEvent.title) {
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p style={{ color: C.textMid }} className="text-sm mb-8">
          Your projected net worth journey and financial milestones
        </p>
        ...
        <div className="flex gap-6 justify-center text-sm">
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 rounded" style={{ backgroundColor: C.teal }}></div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h3 style={{ fontFamily: "'Playfair Display', Georgia, serif" }} className="text-2xl font-bold">
            Financial Events & Milestones
          </h3>
          <button
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEvent({ ...newEvent, month: e.target.value })
                  }
                  style={{
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={newEvent.type}
                  onChange={(e) => {
                    const newType = e.target.value as 'positive' | 'negative' | 'neutral';
                    setNewEvent({
                      ...newEvent,
                      type: newType,
                    });
              ...
              <div>
                ...
                <input
                  type="number"
                  placeholder="Amount"
                  value={newEvent.estimatedImpact}
                  onChange={(e) =>
                    setNewEvent({
                      ...newEvent,
                      estimatedImpact: parseInt(e.target.value),
                    })
                  }
                  style={{
                    backgroundColor: C.bg,
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="px-4 py-2 border rounded-lg font-semibold hover:opacity-80 transition text-sm"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="space-y-3">
          {events.length === 0 ? (
            <p style={{ color: C.textDim }} className="text-center py-8 text-sm">
              No financial events recorded yet. Add one to track your financial milestones.
            </p>
          ) : (
            events.map((event) => (
              <div
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    <span
                      ...
                      style={{
                        backgroundColor: `${getEventColor(event.type)}40`,
                  ...
                  >
                    {event.type === 'positive' && '+'}
                    {formatCurrency(event.estimatedImpact)}
                  </p>
                </div>
                <button
            ...
            {events
              ...
              .map((e) => (
                <div key={e.id} className="flex justify-between text-sm">
                  <span>{e.title}</span>
                  <span style={{ color: C.green }} className="font-semibold">
                    +{formatCurrency(e.estimatedImpact)}
                  </span>
                </div>
              ))}
          </div>
          {events.filter((e) => e.type === 'positive').length === 0 && (
            <p style={{ color: C.textDim }} className="text-sm">
              No positive events recorded
            </p>
          )}
        </div>
          ...
          <div className="space-y-2">
            {events
              ...
              .map((e) => (
                <div key={e.id} className="flex justify-between text-sm">
                  <span>{e.title}</span>
                  <span style={{ color: C.red }} className="font-semibold">
                    {formatCurrency(e.estimatedImpact)}
                  </span>
                </div>
              ))}
          </div>
          {events.filter((e) => e.type === 'negative').length === 0 && (
            <p style={{ color: C.textDim }} className="text-sm">
